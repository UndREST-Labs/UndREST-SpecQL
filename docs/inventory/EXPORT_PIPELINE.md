...
- **`api-index-grouped.json`** — grouped/deduplicated structure (schema `3.1.0`) with optional bounded research metadata.
...
| `--grouped`     | _(off)_                                | Also write grouped/deduplicated `api-index-grouped.json` (schema 3.1.0) |
...
| `inventory/api-index-grouped.json`    | `--grouped`  | Grouped/deduplicated index (schema 3.1.0, recommended for runtime consumers) |
